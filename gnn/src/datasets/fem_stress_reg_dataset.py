...
from src.transforms.stress_as_label_transform import create_transform
...
def dataset_modify_commandline_options(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument('--data_dir', type=str, default=os.path.join('inputs', 'FEM_STRESS_REG'), help='FEMデータを保存する場所')
    return parser
