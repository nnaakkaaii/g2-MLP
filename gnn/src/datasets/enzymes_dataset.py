...
def create_dataset(transform: Any, is_train: bool, opt: argparse.Namespace) -> InMemoryDataset:
    dataset = TUDataset(opt.data_dir, name='ENZYMES', transform=transform, cleaned=opt.cleaned)
...
def dataset_modify_commandline_options(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument('--data_dir', type=str, default=os.path.join('inputs', 'ENZYMES'), help='ENZYMESデータを保存する場所')
