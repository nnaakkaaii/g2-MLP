...
def create_dataset(transform: Any, is_train: bool, fold_number: int, opt: argparse.Namespace) -> InMemoryDataset:
    dataset = TUDataset(opt.data_dir, name='MUTAG', transform=transform, use_node_attr=True)
...
def dataset_modify_commandline_options(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    parser.add_argument('--data_dir', type=str, default=os.path.join('inputs', 'MUTAG'), help='mutagデータを保存する場所')
    parser.add_argument('--index_file_dir', type=str, default=os.path.join('inputs', 'MUTAG', '10fold_idx'), help='mutagデータのindexファイルを保存するディレクトリ')
    return parser
